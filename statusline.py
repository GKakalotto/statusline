#!/usr/bin/env python3
"""
Claude Code Status Line — Linux only.

Displays: model, context usage (●/○ bar), current agent, git branch (cyan=clean, yellow=dirty), cost.
"""
...
def main():
    ...
    if used_pct is not None:
        ...
        used_tokens = (total_input or 0) + (total_output or 0)
        token_info = f"{fmt_tokens(used_tokens)}/{fmt_tokens(window_size)}"
    ...
    # 4. Git branch (cyan = clean, yellow = dirty)
    ...
    parts.append(color(fmt_cost(total_cost), DIM))
